...
algorithm_paper_5 <- function(raw_data, True_labels, 
                              max.iter = 10, stepsize = 0.02,
                              epi = 0.01){
  
  # True_labels should be author.id
  # raw data := list of 3 matices
  n_obs <- nrow(raw_data)
  n_features <- 4 # This number is decided by our selection of features
...
test_our_label <- test_comeon_iamlazy(test_akumar, colMeans(ag5_akumar$best), KK)
